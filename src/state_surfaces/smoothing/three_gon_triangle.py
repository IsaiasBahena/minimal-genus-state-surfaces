...
Triangle (3-gon) smoothing that matches the paper logic.

Key "same-logic" guarantees
---------------------------
...
3) We shift in two stages (exactly like paper):
...
4) We "find triangle pairs" by scanning for triangle->triangle along the component,
   ...
   NOTE: word/segment is NOT filtered by smoothed crossings (matches paper).
...
6) We add EXACTLY ONE triangle state circle to smoothed_pairs_copy (matches paper).
...
# ---------------------------------------------------------------------
# Shared helpers (keep consistent with other modules)
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# Stage: build triangle pairs (same as paper scanning loops)
# ---------------------------------------------------------------------
...
def _find_triangle_pairs_in_components(components: List[List[int]], triangle_crossings: Tuple[int, int, int],) -> List[Tuple[int, List[int], int, int]]:
    """
    Find triangle pairs by scanning each component, reusing paper logic.
...
def _chain_pairs_to_components(pairs: List[Tuple[int, List[int], int, int]]) -> List[List[int]]:
    """
    Chain triangle pairs into smoothed components (matches paper Step 5).
...
def _build_triangle_state_circle_from_smooth_copy(smooth_copy: List[List[int]], triangle_crossings: Tuple[int, int, int],) -> Tuple[int, ...]:
    ...
    # Convert to "smoothed-pair" format used in paper: (start, segment, end, comp_idx)
    triangle_smoothed_pairs = tri_pairs

    # Chain into a single state circle (paper breaks after first)
    visited: Set[int] = set()
    pair_map: Dict[int, List[Tuple[int, Tuple[int, List[int], int, int]]]] = defaultdict(list)
...
# ---------------------------------------------------------------------
# Public API (signature mirrors the paper function)
# ---------------------------------------------------------------------
...
def smooth_3_gon_triangle(gauss_code: GaussCode, triangle_crossings: Tuple[int, int, int], smoothed_crossings: List[int], smoothed_pairs: List[Tuple[int, ...]], c1: int, i1: int, j1: int, c2: int, i2: int, j2: int, c3: int, i3: int, j3: int,) -> Tuple[List[List[int]], List[int], List[Tuple[int, ...]]]:
    """
    Triangle smoothing branch (matches paper development code).
    ...
    smoothed_crossings:
        existing list of smoothed crossings (mutated like paper).
    smoothed_pairs:
    ...
    # ------------------------------------------------------------
    # Step 2 (paper): shift first triangle crossing of each pair to the front
    # ------------------------------------------------------------
    shift_map: Dict[int, Tuple[int, Tuple[int, int]]] = {}
    for comp_idx, start_idx, end_idx in [(c1, i1, j1), (c2, i2, j2), (c3, i3, j3)]:
        # paper keeps the smallest start_idx per component if repeated
    ...
    # ------------------------------------------------------------
    # Step 3 (paper): shift the second triangle crossing of each pair to the front
    # ------------------------------------------------------------
    # IMPORTANT: paper uses the ORIGINAL gauss_code indices to identify
    ...
    for comp_idx, (_shift_idx, (i, j)) in shift_map.items():
        ...
        try:
            ...
            # In paper print, here just continue safely.
            pass

    # ------------------------------------------------------------
    # Step 4 + 5 (paper): identify triangle pairs in code_copy and chain them
    # ------------------------------------------------------------
    ...
    # ------------------------------------------------------------
    # Step 6 + 7 (paper): build triangle state circle from smooth_copy
    # ------------------------------------------------------------
    ...
    # ------------------------------------------------------------
    # Step 8 (paper): retain unaffected components from ORIGINAL gauss_code
    # ------------------------------------------------------------
    affected = {c1, c2, c3}
    ...
    # ------------------------------------------------------------
    # Step 9 (paper): update smoothed crossings with triangle endpoints
    # ------------------------------------------------------------
